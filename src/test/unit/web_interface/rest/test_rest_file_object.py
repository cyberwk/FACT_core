...
def test_bad_query(test_client):
    ...
    result = test_client.get('/rest/file_object?query={}'.format(quote(bad_json_document))).json
...
def test_successful_request(test_client):
    result = test_client.get('/rest/file_object/{}'.format(TEST_TEXT_FILE.uid)).json
